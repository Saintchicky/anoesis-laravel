...
<div class="title_page" id="Up"><h1>PLAN</h1></div>
        ...
        <div class="row d-flex justify-content-center">
           ...
           {{$service->TitleService->titre_services}}
               ...
               @foreach ( $services_details as $service )
                @if($service->TitleService->id == 1)
               <li> <form id="frm" action="{{route('serviceUpStore',['id'=>$service->id])}}"  class="form-inline" method="POST">
                  @csrf
                  
                     <div class="input-group">
              
                     <input class="form-control" type="text" id="service" name="service" value="{{$service->service or old ('service')}}">
                     
                        
                     </div>
                     <div class="input-group">
                      <button id="sendToDB" type="submit" class="btn btn-outline-success btn-sm">Sauvegarder</button>
                      <span><a href="{{route('deleteService',['id'=>$service->id])}}" type="button" class="btn btn-outline-danger btn-sm">Supprimer</a></span>
                     </div>
              </form></li>
